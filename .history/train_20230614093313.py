...
def train(model, train_dataloader, test_dataloader, optimizer, scheduler, epoches):
    ...
    for epoch in range(epoches):
        ...
        torch.save(model.state_dict(), os.path.join('./models/mixup_ad_adaptation_id3', str(epoch+1) + '.pth'))
        # if epoch == 2:
        ...
        if accuracy > best_accu:
            torch.save(test_model.state_dict(), os.path.join('./models/mixup_ad_adaptation_id3', 'best_'+str(args.person_id)+'.pth'))
            best_accu = accuracy
